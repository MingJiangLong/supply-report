import { API_HOST, isDev } from "@/config"
...
axios.interceptors.response.use(
  async data => {
    if (
      data?.data?.head?.code == 403 &&
      !data.config.headers["retry_token_done"]
    ) {
    ...
    if (data?.data?.head?.code != 200)
      throw new Error(data?.data?.head?.desc ?? "服务器异常")
    return data.data
  },
  async error => {
...
export function fetchGoodsInMachine(vmCode: string) {
  return post(
    `/simple/adjust_channel/get_channel_goods_list`,
    {
      vmCode,
    },
    {
...
export function fetchUploadInfo(): Promise<
  Response<{
    accessKeyId: string
